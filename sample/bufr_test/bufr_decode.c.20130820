...
//  long ktdlst[KELEM],ktdexp[KELEM],ktdlen,ktdexl;
  ...
  /////////////////////////////////////////// user add
  long isubset =0;
  int isubsetcnt =0;				// count of subset in one message
  int itotalsubsetcnt=0;		// total count of subset
  int imsgcnt=0;						// total count of message
  ///////////////////////////////////////////
  int kreq[2];
  int krql=1;
  int krq[krql];
...
/*
    if (ksup[5] > 1)
    {
      ...
      kelem = KELEM;
*/
		printf("ksec3[1] : %d, imsgcnt:%d\n",ksec3[1], imsgcnt);
		isubsetcnt = ksec3[1];
    if (isubsetcnt > 1)
    {
      kelem = kvals/isubsetcnt;
      printf("ksup[5]\n");
    }
    else
      kelem = KELEM;
...
//		printf(">>>>>>>>>>>>>>>>>>>ksec3:%d,%d,%d,%d\n",ksec3[0],ksec3[1],ksec3[2],ksec3[3]);
    ...
    icode = 0;
    current_ss = 1;
    current_ss_end = isubsetcnt;//128;//ksup[5];
    buprt_(&icode,&current_ss,&current_ss_end,&kelem,(char **)cnames,(char **)cunits,(char **)cvals,&kvals,values,ksup,ksec1,&kerr);
//    printf("buprt_ %d\n", kerr);
//		printf("buprt_ next\n");

  	imsgcnt ++;
  	itotalsubsetcnt = itotalsubsetcnt + isubsetcnt;
      
  }
  
  printf("total number of subset  : %d\n", itotalsubsetcnt);
  printf("total number of message : %d\n", imsgcnt);
